...
OrderedMap.prototype = {
  ...
  subtract: function(map2) {
    var result = this;
    map2 = OrderedMap.from(map2);
    for (var i = 0; i < map2.content.length; i += 2) result = result.remove(map2.content[i]);
    return result;
  },
  // :: () → Object
...
var ParseContext = class {
  ...
  addTextNode(dom, marks2) {
    ...
    if (preserveWS === "full" || top.inlineContext(dom) || /[^ \t\r\n\u000c]/.test(value)) {
      ...
      if (value) this.insertNode(this.parser.schema.text(value), marks2, !/\S/.test(value));
      this.findInText(dom);
    } else {
      this.findInside(dom);
    }
    ...
    if (toRemove && toRemove.length) {
      ...
      for (let i = 0; i < toRemove.length; i++) {
        ...
        for (let j = 0; j < matched.length; j++) {
          let m = matched[j];
          if (m.step == step - 1 && style.eq(matched[j].style)) found2 = m;
        }
        if (found2) {
          found2.to = end;
          found2.step = step;
...
function parseFromClipboard(view, text2, html, plainText, $context) {
  ...
  if (asText) {
    ...
    if (inCode) return text2 ? new Slice(Fragment.from(view.state.schema.text(text2.replace(/\r\n?/g, "\n"))), 0, 0) : Slice.empty;
...
function toggleMark(markType, attrs = null, options) {
  ...
  return function(state, dispatch) {
    ...
    if (empty2 && !$cursor || !markApplies(state.doc, ranges, markType, enterAtoms)) return false;
    if (dispatch) {
      if ($cursor) {
...
var MenuComponent = class _MenuComponent {
  ...
  static ɵfac = function MenuComponent_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _MenuComponent)(ɵɵdirectiveInject(MenuService));
  };
  static ɵcmp = ɵɵdefineComponent({
    type: _MenuComponent,
...
var FloatingMenuComponent = class _FloatingMenuComponent {
  ...
  static ɵfac = function FloatingMenuComponent_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _FloatingMenuComponent)(ɵɵdirectiveInject(ElementRef));
  };
  static ɵcmp = ɵɵdefineComponent({
